...
export default function Header() {
  ...
  useEffect(() => {
    ...
    if (modoClaro) {
      ...
      root.style.setProperty("--clr-acentado-color", "349 100% 10%");
...
// El estado modoClaro se utiliza para rastrear si el modo claro está activado o no. El primer useEffect verifica las preferencias del sistema del usuario para el esquema de colores (claro u oscuro) y establece el estado modoClaro segun las preferencias del sistema. El segundo useEffect se activa cuando el estado modoClaro cambia. Si modoClaro es true, se establecen las variables CSS para el tema claro. Si modoClaro es false, se establecen las variables CSS para el tema oscuro.
// El segundo useEffect se activa cuando el estado modoClaro cambia. Si modoClaro es true, se establecen las variables CSS para el tema claro. Si modoClaro es false, se establecen las variables CSS para el tema oscuro. El botón de cambio de color tiene un id único para identificarlo. Cuando se hace clic en el botón, se llama a la función setModoClaro con el valor opuesto de modoClaro. Si modoClaro es true, se renderiza el icono de la luna. Si modoClaro es false, se renderiza el icono del sol.
// AnimatePresence: Es un componente de la biblioteca framer-motion que permite animaciones de entrada y salida para los componentes  que se montan y desmontan.  Si estaMenuAbierto es true, el componente NavMenu se renderiza. Si es false, NavMenu no se renderiza. toggleMenuAbierto es una función que se pasa al componente NavMenu como prop
